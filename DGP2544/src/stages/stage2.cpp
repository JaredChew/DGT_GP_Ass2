...
Stage_2::Stage_2(GameWindow& wnd, ResourceLoader& rseLoad, Player& player, Audio& audio, Physics& physics, Keyboard& kbd) : wnd(wnd), rseLoad(rseLoad), player(player), audio(audio), physics(physics), kbd(kbd) {

	// ** Initialise everything here ** //

	std::cout << "\nSetting up Stage 2" << std::endl;
	...
	bgm = rseLoad.loadMusic("asset/music/stage2.flac");
	audio.music.playMusic(bgm, -1);

	gameOver_bgm = rseLoad.loadMusic("asset/music/gameOver.flac");

	//Load background
	background.bgImg = rseLoad.loadTextureImage("asset/graphic/background2_scroll.png");
...
Stage_2::~Stage_2() {
	...
	std::cout << "Stage 2 objects cleaned" << std::endl;
...
void Stage_2::enemyAI() {
	...
	for (int i = 0; i < MAX_MINI_ENEMY; i++) {
		...
		else if (miniEnemy[i]->getPosY() > HEIGHT) {
		...
		else if (miniEnemy[i]->getAutoMoveDown()) {
			miniEnemy[i]->moveDown();
		}

		//Mini enemy flys away after passing player
...
void Stage_2::collisionDetection() {
...
bool Stage_2::getStageClear() {
	return stageClear;
}

void Stage_2::Background::scroll(GameWindow& wnd) {
...
void Stage_2::Background::render(int x, int y, GameWindow& wnd) {
